...
| Cookie 管理 | CookieManager → CookieStore | 返回 cookie 字符串 | 无 |
...
3. **POST**: `WebView.loadUrl()` 只支持 GET。阅读对 POST 的处理是先 OkHttp POST，再把响应 HTML 加载到 WebView。P9 暂不实现此路径。
4. **Cookie**: 阅读把 cookie 存入 Room DB 做持久化。P9 只返回 cookie 字符串，不做持久化。
